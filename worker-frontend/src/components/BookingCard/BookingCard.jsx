...
export default function BookingCard({
  ...
  /*
   * These are internal service identifiers only.
   * The visible service name always comes from
  ...
  /*
   * Normalize all possible service identifiers.
   */
  ...
  const normalizedPrimaryService =
    normalizeServiceKey(
  ...
  /*
   * Prefer a valid service key found in the
   * booking title.
   *
   * This handles older/mismatched records such as:
   *
   * serviceCategory: "appliance_repair"
   * title: "electrician"
   *
   * In that situation the title clearly identifies
   * the actual requested service.
   */
  const serviceKey =
    serviceKeys.includes(normalizedTitle)
      ? normalizedTitle
      : serviceKeys.includes(
  ...
  /*
   * Translate the internal service key using the
   * currently selected Worker language.
   */
  const serviceName =
    serviceKey
  ...
  /*
   * If the booking title itself is just a service key
   * such as "electrician", do not show that raw key
   * as the main request title.
   *
   * Use the translated service name instead.
   */
  const titleIsServiceKey =
    serviceKeys.includes(
      normalizedTitle
    );

  const titleIsTranslatedService =
    serviceKeys.some((key) => {
