...
export default function MyBookingsPage() {
  ...
  // Status modal
  const [actionTarget, setActionTarget] = useState(null); // { id, newStatus }
  const [isProcessing, setIsProcessing] = useState(false);

  const fetchMyBookings = async () => {
    try {
      setIsLoading(true);
      const data = await bookingsGateway.getMyBookings();
      setBookings(data);
    } catch (err) {
  ...
  const filteredBookings = useMemo(() => {
    let list = bookings.filter((b) => {
      if (Array.isArray(currentTabConfig.statusFilter)) {
        return currentTabConfig.statusFilter.includes(b.status);
      }
      return b.status === currentTabConfig.statusFilter;
    });
    ...
    return list;
  }, [bookings, activeTab, currentTabConfig, searchQuery]);

  const handleStatusChange = async (bookingId, newStatus) => {
    setActionTarget({ id: bookingId, newStatus });
  };
  ...
  const handleConfirmStatus = async () => {
    ...
    try {
      ...
      await bookingsGateway.updateBookingStatus(actionTarget.id, actionTarget.newStatus);
      showToast(
  ...
  const getEmptyStateConfig = () => {
    if (activeTab === 'upcoming') {
      return {
        title: t('myBookings.emptyUpcomingTitle'),
        description: t('myBookings.emptyUpcomingDesc'),
        actionText: t('myBookings.findJobsBtn'),
        onAction: () => navigate('/available-jobs'),
      };
    }
    if (activeTab === 'active') {
      return {
        title: t('myBookings.emptyActiveTitle'),
        description: t('myBookings.emptyActiveDesc'),
      };
    }
    if (activeTab === 'completed') {
      return {
        title: t('myBookings.emptyCompletedTitle'),
        description: t('myBookings.emptyCompletedDesc'),
      };
    }
    return {
      title: t('myBookings.emptyCancelledTitle'),
      description: t('myBookings.emptyCancelledDesc'),
    };
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 overflow-x-auto scrollbar-none">
          {tabs.map((tab) => {
            ...
            const count = bookings.filter((b) =>
              Array.isArray(tab.statusFilter)
                ? tab.statusFilter.includes(b.status)
                : b.status === tab.statusFilter
            ).length;
      ...
      {/* Bookings List */}
      {isLoading ? (
        <LoadingState message="Loading your assigned bookings..." />
      ...
      <ConfirmDialog
        isOpen={!!actionTarget}
        onClose={() => !isProcessing && setActionTarget(null)}
        ...
        confirmText={
          actionTarget?.newStatus === 'completed' ? 'Complete & Record' : 'Start Job'
        }
        type={actionTarget?.newStatus === 'completed' ? 'success' : 'info'}
      />
    </div>
  );
}
