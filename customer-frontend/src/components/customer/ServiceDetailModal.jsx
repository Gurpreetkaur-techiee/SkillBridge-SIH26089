...
export function ServiceDetailModal() {
  ...
  const [selectedTimeSlot, setSelectedTimeSlot] =
    useState('Immediate (15-30 mins)');

  const [notes, setNotes] = useState('');
  const [isSuccess, setIsSuccess] = useState(false);

  const [createdBookingData, setCreatedBookingData] =
    useState(null);
   ...
   *
   * Then create the Firestore booking and WAIT for
   * Firestore to confirm that the booking was created.
   *
   * This prevents the UI from showing a successful
   * booking before the database write has completed.
   */
  const handlePaymentSuccess = async (receipt) => {
    try {
      ...
      const booking = await createBooking(
        {
          ...
          /*
           * Keep the selected task as the request title
           * when the customer has selected one.
           */
          title:
            selectedTask ||
          ...
          /*
           * Use the paid base amount when available.
           */
          price:
            receipt?.baseAmount ||
        ...
        /*
         * IMPORTANT:
         * Keep the selected worker attached to the booking.
         *
         * createBooking() uses this value to store
         * the worker Firebase UID.
         */
        selectedWorker,
      ...
      /*
       * Firestore booking has now been created.
       * Only now show the success state.
       */
      ...
      /*
       * Do not show "Booking Confirmed" when
  ...
  return (
    <Modal
      ...
      subtitle={
        isSuccess
          ? 'A verified pro is being dispatched to your location'
          : 'Customize your service requirements and complete checkout'
      }
      maxWidth="max-w-lg"
    >
      {isSuccess ? (
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl text-left space-y-2 text-xs border border-slate-200/80 dark:border-slate-700/60">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-slate-900 dark:text-white">
                {createdBookingData?.serviceName ||
                  createdBookingData?.title ||
                  selectedService.title}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                ...
                Paid via{' '}
                {createdBookingData?.paymentMethod}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-slate-700 dark:text-slate-300">
                {createdBookingData?.transactionId}
              </span>
            </div>
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex justify-between">
              ...
              <span className="font-extrabold text-sm text-blue-600 dark:text-blue-400">
                {createdBookingData?.totalPaid ||
                  createdBookingData?.amount}
              </span>
            </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/40">
            ...
            <div className="flex items-center gap-1.5 text-xs">
              ...
              <span className="font-bold text-slate-800 dark:text-slate-200">
                {selectedService.rating || 4.9}
              </span>

              <span className="text-slate-400">
                ({selectedService.reviewCount || 200}+ reviews)
              </span>
