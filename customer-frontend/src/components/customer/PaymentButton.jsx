...
export function PaymentButton({
  ...
  // Format numerical amount cleanly
  const numericAmount = typeof amount === 'string' 
    ? parseFloat(amount.replace(/[^0-9.]/g, '')) || 35 
    : Number(amount) || 35;

  const platformFee = 2.50;
  const totalAmount = (numericAmount + platformFee).toFixed(2);
  ...
  const handleProcessPayment = () => {
    ...
    if (paymentMethod === 'card') {
      if (!cardNumber || !cardExpiry || !cardCvc || !cardName) {
        setPaymentError('Please fill in complete card billing details.');
        return;
      }
    } else if (paymentMethod === 'upi') {
      if (!upiId || !upiId.includes('@')) {
        setPaymentError('Please enter a valid UPI / VPA ID.');
        return;
      }
    }
    ...
    // Simulate secure bank gateway transaction processing
    setTimeout(() => {
      ...
      const paymentReceipt = {
        ...
        paymentMethod: paymentMethod === 'card' ? 'Credit/Debit Card' : paymentMethod === 'upi' ? 'UPI' : paymentMethod === 'wallet' ? 'Google/Apple Pay' : 'Cash on Completion',
        status: 'PAID',
  ...
  return (
    <>
      ...
      <Button
        ...
        icon={CreditCard}
      >
        {buttonText || `Pay Now • $${numericAmount}`}
      </Button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-10 animate-scale-up">
            ...
            <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 space-y-2 text-xs">
                <div className="flex justify-between items-center text-slate-600 dark:text-slate-300">
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{serviceName}</span>
                  <span>${numericAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center text-slate-500 dark:text-slate-400">
                  <span>Platform Fee & Trust Guarantee</span>
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-sm font-extrabold text-slate-900 dark:text-white">
                  ...
                  <span className="text-base text-blue-600 dark:text-blue-400">${totalAmount}</span>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/40 flex items-start gap-2.5">
                ...
                <p className="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
                  <strong className="text-blue-700 dark:text-blue-300">SkillBridge Escrow Protection:</strong> Funds are safely held in escrow and will only be released to the worker when you confirm job completion.
                </p>
              </div>
            ...
            <div className="p-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
              ...
              >
                {paymentMethod === 'cash' ? 'Confirm Booking' : `Authorize & Pay $${totalAmount}`}
              </Button>
            </div>
